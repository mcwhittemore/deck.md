...
var convertFiles = function(html, paths, urls, index, includeLocals, callback){
	var url = urls[index];
	...
	var result = function(err, res, body){
		...
		if(index<urls.length-1){
			convertFiles(html, paths, urls, index+1, includeLocals, callback);
		}
		else{
...
var findExternalFiles = function(html, callback){
	...
	if(urls.length>0){
		convertFiles(html, paths, urls, 0, true, callback);
	}
	else{
		callback(undefined, html);
	}
...
exports.render = function(md, userConfig, callback){
	...
	jade.render(jd, {filename: jd_file, locals: config}, jadeback);

}

exports.renderFile(md_file, {}, function(err, html){
	console.log("DONE");
	fs.writeFile(ht_file, html, "utf8");
});